...
func (p *WasmNFTPatcher) Run() error {
	p.logger.Info("[Patch v1.0.11] Starting Wasm NFT data recovery")
	...
	p.logger.Info("[Patch v1.0.11] Step 1: Scanning transactions", slog.Int64("total", totalCount))

	p.logger.Info("[Patch v1.0.11] Step 2: Clearing NFT data only")
	if err := p.clearNFT(); err != nil {
		return err
	}

	p.logger.Info("[Patch v1.0.11] Step 3: Processing transactions in batches")
	...
	p.logger.Info("[Patch v1.0.11] Wasm NFT data recovery completed",
...
func (p *WasmNFTPatcher) processWasmEvent(
	...
	switch action {
	case wasm_nft.EventAttrNftMint:
	...
	case wasm_nft.EventAttrNftTransfer, wasm_nft.EventAttrNftSend:
		tokenId, found := attrMap["token_id"]
		if !found {
			return
		}
	...
	case wasm_nft.EventAttrNftBurn:
		tokenId, found := attrMap["token_id"]
		if !found {
			return
		}
...
func (p *WasmNFTPatcher) handleNftTransations(nftTxMap map[string]map[string]map[string]interface{}) error {
	...
	for txHash, collectionMap := range nftTxMap {
		...
		if err != nil {
			log.Printf("[Patch v1.0.11] Failed to decode tx hash: %s, error: %v", txHash, err)
			continue
		}
